...
import { useNotifications as useNotificationsHook, useMarkNotificationAsRead, useMarkAllNotificationsAsRead, useDeleteNotification } from '@/state';
...
export default function NotificationsPage() {
  const { data: queryNotifications = [] } = useNotificationsHook();
  const markAsReadMutation = useMarkNotificationAsRead();
  const markAllMutation = useMarkAllNotificationsAsRead();
  const deleteMutation = useDeleteNotification();
  const [filter, setFilter] = useState<'all' | 'unread'>('all');

  // Adapt new model field names to what template expects
  const notifications = queryNotifications
    .map(n => ({ ...n, timestamp: (n as any).createdAt ?? (n as any).timestamp ?? n.createdAt }));
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {notification.link && (
                              <Link to={notification.link} onClick={() => !notification.isRead && markAsRead(notification.id)}>
