...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    if (categoryId) {
      const selectedCategoryNormalized = normalizeCategoryText(category?.name || categoryId);
      result = result.filter((p) => {
        const productCategoryNormalized = normalizeCategoryText(p.category);
        return (
          p.category === categoryId ||
          p.category.toLowerCase().replace(/\s+/g, '-') === categoryId.toLowerCase() ||
          productCategoryNormalized === selectedCategoryNormalized
        );
      });
    }
  ...
  }, [categoryId, category?.name, sortBy, products]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12">
          ...
          <div className="flex flex-wrap gap-3">
            {categoriesData.filter(c => c.id !== categoryId).map(cat => (
              <Link
                key={cat.id}
                to={`/category/${cat.id}`}
